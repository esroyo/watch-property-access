...
function _watchAccess<T extends object>(target: T, onlyOwnProperty: boolean, registry: Registry, path: string): TraceAccess<T> {
  return new Proxy(target, {
     get(...args) {
         ...
         if (shouldRegister) {
             const fullPath = `${path}${isNumeric(property) ? '[]' : String(property)}`;
             ...
             if (value && (valueType === 'object' || valueType === 'function')) {
                 ...
                 return _watchAccess(value, onlyOwnProperty, registry, `${fullPath}.`);
             }
         }
         return value;
     },
...
export function watchPropertyAccess<T extends object>(target: T, onlyOwnProperty = true): TraceAccess<T> {
